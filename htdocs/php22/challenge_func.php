...
function calc_bmi($height, $weight){
    if(strlen($height) !== mb_strlen($height)){
        $height = mb_convert_kana($height,'n');
    }
    if(strlen($weight) !== mb_strlen($weight)){
...
// check_float関数作成
/////////////////////
function check_float($param){
    if(strlen($param) !==  mb_strlen($param)){
        $param = mb_convert_kana($param,'n');
    }
    if(is_numeric($param) === TRUE && ctype_digit($param) === FALSE){
        $param = floatval($param);
        $bool = $param > 0;
        return $bool;
    }
    elseif(is_numeric($param) === TRUE && ctype_digit($param) === TRUE){
        $param = intval($param);
        $bool = $param > 0;
        return $bool;
    }
